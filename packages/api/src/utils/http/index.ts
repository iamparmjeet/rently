...
// Define a single object with all HTTP status codes
...
// Define a single object with all HTTP status phrases
...
export const getReasonPhrase = (code: number): string => {
  const key = Object.entries(StatusCode).find(
    ([_, value]) => value === code
  )?.[0];
  return key
    ? StatusPhrase[key as keyof typeof StatusPhrase]
    : "Unknown Status";
};
