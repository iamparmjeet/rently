		...
		<header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60">
			<div className="container mx-auto flex h-16 max-w-4xl items-center justify-between px-4">
				...
				<div className="flex items-center gap-3">
					<Logo />
					<span className="hidden flex-row border-l pl-3 text-muted-foreground text-sm sm:block">
						Tenant Portal
					</span>
				</div>
				...
				<div className="flex items-center gap-3">
					...
					{session?.user && (
						<div className="relative">
							...
							{/* WHY: bottom-right badge is the universal pattern (think LinkedIn, Gmail) */}
							<div className="absolute -right-0.5 -bottom-0.5">
								{isVerified ? (
									<IconCircleCheckFilled className="h-4 w-4 text-emerald-500" />
								) : (
									<IconMailQuestion className="h-4 w-4 text-amber-500" />
								)}
							</div>
						</div>
					)}
